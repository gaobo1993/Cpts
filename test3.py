...
axes( None, clr.black, 30, (-110,40,0))
...
density = [0 for i in range( 121*121 )]

def getDensity(pos):
    return density[(pos[0]+60)*121+(pos[1]+60)]


# Set the environment here

def drawEnv( sourcePos = (-40, 0, 0) ):
    for i in range(-60, 60):
        for j in range(-60, 60):
            num = (i+60)*121 + (j+60)
            p = (i, j, 0)            
            density[num] = 1 / (dist((-40, 0, 0), p)+0.00001)
            if density[num] < 1:
                vs.sphere(pos = p, radius = 0.5, color = clr.black, opacity = density[num])
            else:
...
def loopVehicle( initPos = (40, 40, 0), theta = 3/2*pi, delta = 0.5, W = 4 ):
    """ Init & loop vehicle.
    """
    
    initAxis = (cos(theta), sin(theta), 0)
    ...
    while True:
        ...
        vehicle.velocity = makeHoriVector(theta)
    
        vehicle.pos = vehicle.pos+vehicle.velocity*deltat*sigmoid(getDensity(lSensorPos)
                                                    +getDensity(rSensorPos))

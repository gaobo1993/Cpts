...
axes( None, clr.black, 30, (-80,-20,0))
...
def loopVehicle( initPos = (40, 40, 0), theta = 3/2*pi, phi = 0, delta = 0.5, alpha = -4, W = 4, maxIter = 2000):
    ...
    while True:
        ...
        if (getDensity(lSensorPos) == -1 or getDensity(rSensorPos) == -1 or
            ...
            vehicle.pos = vehicle.pos+vehicle.velocity*deltat*W
            vehicle.trail_object.radius = 0.2
        ...
        vehicle.pos = vehicle.pos+vehicle.velocity*deltat*sigmoid(getDensity(lSensorPos)+getDensity(rSensorPos)+getDensity(hSensorPos)+getDensity(lowSensorPos), alpha)
        vehicle.axis = vehicle.velocity
        vehicle.size=(W,W,0.2)

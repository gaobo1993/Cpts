...
def loopVehicle( vsrcPos = vs.vector(-20, 0, 30), initPos = (40, 40, 0), theta = 3/2*pi, phi = 0, delta = 0.5, W = 4, alpha = -4, thre_den = 0.3 ):
    """ Init & loop vehicle.
    """
    ...
    while True:
        ...
        if (getDensity(lSensorPos)+getDensity(rSensorPos)+getDensity(hSensorPos)+getDensity(lowSensorPos))/4 <= thre_den:
            time = time + 1
        else:
...
thre_den = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]
...
for k in range(len(thre_den)):
    for i in range(1, 4):
##        for j in range(1, 3):
        temp = loopVehicle(vsrcPos = vs.vector(-20, 0, 30), theta = i/2*pi, initPos = (20, 20, 45), alpha = 0, delta = 1, thre_den = thre_den[k])
...
ax2.legend(loc = 2)
plt.grid()
plt.savefig('./ROBIO/3d_t_d.png', bbox_inches='tight')
